...
do
    ...
    then
        ...
        then
            ...
            helm upgrade --version 8.0.0 --timeout 600s --reuse-values redis --set "password=$REDIS_PASSWORD,cluster.update.addNodes=true,cluster.update.currentNumberOfNodes=$current_num_nodes,cluster.nodes=$new_num_nodes,updateJob.nodeType=slave,updateJob.masterID=$master_id" bitnami/redis-cluster -n redis
            exit 0
        elif [[ "$node_type" == "M" ]]
        then
            ...
            echo "Number of cluster nodes after scaling = $new_num_nodes"
            helm repo add bitnami https://raw.githubusercontent.com/bitnami/charts/archive-full-index/bitnami/
            helm upgrade  --version 8.0.0 --timeout 600s --reuse-values redis --set "password=$REDIS_PASSWORD,cluster.update.addNodes=true,cluster.update.currentNumberOfNodes=$current_num_nodes,cluster.nodes=$new_num_nodes,updateJob.nodeType=master" bitnami/redis-cluster -n redis
            sleep 5m
