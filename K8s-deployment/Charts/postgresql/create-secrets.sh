...
echo -n  $(cat /dev/urandom | tr -dc 'a-zA-Z0-9-!@#$%^&*+|:<>?' | head -c 16) > secrets/passwords/repmgr-password
...
echo -n "$(cat secrets/postgresql-password);$(cat secrets/postgres-rs-password);$(cat secrets/postgres-auth-password);$(cat secrets/postgres-keycloak-password)" > secrets/passwords/passwords
